...
public class AnnotationMatch {
    protected static final Logger logger = Logger.getLogger(AnnotationMatch.class.getSimpleName());
    
    /**
     * Returns cloze sentences where certain nouns are considered as key.
     * @param sentences
     * @param firstSentence
     * @return
     */
        ...
        switch (sentences.get(0).getLanguage()) {
            case DE:
                ...
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(germanGapFinder.findAnnotations(sentences.get(i), false));
                }
                return clozeSentences;
            case FR:
                FrenchGapFinder frenchGapFinder = new FrenchGapFinder(0);
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(frenchGapFinder.findAnnotations(sentences.get(i), false));
                }
                return clozeSentences;
            case EN:
                EnglishGapFinder englishGapFinder = new EnglishGapFinder(0);
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(englishGapFinder.findAnnotations(sentences.get(i), false));
                }
                return clozeSentences;
            default:
