 ...
 */
public class Noun {
    protected static final Logger logger = Logger.getLogger(Noun.class.getSimpleName());

    /**
     * Returns cloze sentences where certain nouns are considered as key-distractor pairs.
     * @param sentences
        ...
        switch (sentences.get(0).getLanguage()) {
            case DE:
                ...
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(germanGapFinder.findNouns(sentences.get(i), true));
                }
                return clozeSentences;
            case FR:
                FrenchGapFinder frenchGapFinder = new FrenchGapFinder(numberOfDistractors);
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(frenchGapFinder.findNouns(sentences.get(i), true));
                }
                return clozeSentences;
            case EN:
                EnglishGapFinder englishGapFinder = new EnglishGapFinder(numberOfDistractors);
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(englishGapFinder.findNouns(sentences.get(i), true));
                }
                return clozeSentences;
            default:
