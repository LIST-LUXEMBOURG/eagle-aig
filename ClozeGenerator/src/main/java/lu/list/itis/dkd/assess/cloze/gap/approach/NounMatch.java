...
public class NounMatch {
    protected static final Logger logger = Logger.getLogger(NounMatch.class.getSimpleName());
        ...
        switch (sentences.get(0).getLanguage()) {
            case DE:
                ...
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(germanGapFinder.findNouns(sentences.get(i), false));
                }
                return clozeSentences;
            case FR:
                FrenchGapFinder frenchGapFinder = new FrenchGapFinder(0);
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(frenchGapFinder.findNouns(sentences.get(i), false));
                }
                return clozeSentences;
            case EN:
                EnglishGapFinder englishGapFinder = new EnglishGapFinder(0);
                for (int i = firstSentence; i < sentences.size(); i++) {
                    clozeSentences.add(englishGapFinder.findNouns(sentences.get(i), false));
                }
                return clozeSentences;
            default:
